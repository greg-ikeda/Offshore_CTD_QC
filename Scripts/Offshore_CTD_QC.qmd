...
if(length(stnddev_df) > 0){
  ...
  # }
  print("Dates with standard deviations errors:")
  print(tibble(sd_error_dates), n = Inf)
} else{
...
## 4. Upcast / Downcast Comparison 

```{r Upcast Downcast Test}
#| label: upcast-downcast-test
#| lightbox:
#|   group: upcast-downcast
#| layout-ncol: 5
#| warning: false
#| message: false
#| code-fold: true
...
source(here("scripts", "Offshore_CTD_UpcastDowncast.R"))

if(nrow(updown_flagged) == 0){
  print("All upcast/downcast data are within reasonable range")
} else{
  updown_rej_plot
}

updown_error_dates <- paste0(unique(updown_flagged$Date))

if(length(updown_error_dates) > 0){
  allplots <- vector("list")
  count <- 0
  
  for(cast in updown_error_dates){
    count <- count + 1
    cast_doi <- ymd(cast)
    profile_of_interest <- updown_df %>%
      filter(Date == cast_doi)
    
    allplots[[count]] <- plot_errors_multipanel_updown(profile_of_interest, cast_doi)
    print(allplots[[count]])
  }
} else{
  print("All upcast and downcast data are comparable")
}
```
